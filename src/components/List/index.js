...
function List({ windowItem, windowObj }) {
  ...
  const children = windowItem["children"]
    .map((child) => {
      ...
      if (
        !isEditModeOn &&
        child["html"].indexOf("true") === 21 &&
        child["html"].indexOf("show") !== -1
      ) {
        child["html"] = child["html"].replace("true", "false");
        child["html"] = child["html"].replace("show", "hide");
      } else if (
        isEditModeOn &&
        child["html"].indexOf("false") === 21 &&
        child["html"].indexOf("hide") !== -1
      ) {
  ...
  useEffect(() => {
      ...
      for (let i = 0; i < listItems.length; i++) {
        ...
        if (!currentListItem.onblur) {
          currentListItem.onblur = function (e) {
            ...
            const strikethroughClass = currentListItem.classList.contains(
              "strikethrough"
            )
              ? "strikethrough"
              : "";
            const strikethroughButtonText = currentListItem.classList.contains(
              "strikethrough"
            )
    ...
    const deleteButtons =
      list.current.getElementsByClassName("list-delete-button");
    if (deleteButtons) {
    ...
    const strikethroughButtons = list.current.getElementsByClassName(
      "list-strikethrough-button"
    );
  ...
  function handleClick() {
    ...
    let newId = 0;
    if (tempChildren.length > 0) newId = getMaxId(windowItem) + 1;
  ...
  return (
    <div>
      <StyledList ref={list}></StyledList>
      {isEditModeOn && (
        <section>
